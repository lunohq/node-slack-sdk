...
RTMClient.prototype._onStart = function _onStart(err, data) {
  ...
  if (err || !data.url) {
    ...
    if (!isUndefined(this.dataStore)) {
      promise = this.dataStore.cacheRtmStart(data);
    }
    promise
      .then(function _onStartPromiseInner() {
        self.emit(CLIENT_EVENTS.AUTHENTICATED, data);
        self.connect(data.url);
      });
  }
};
...
RTMClient.prototype._handleWsMessageViaEventHandler = function _handleWsMessageViaEventHandler(
  ...
  if (!isUndefined(this.dataStore)) {
    promise = this.dataStore.handleRtmMessage(
      this.activeUserId,
      this.activeTeamId,
      messageType,
      message
  ...
  promise.then(function _handleWsMessageViaEventHandlerPromiseInner() {
    if (replyTo) {
      if (!messageType) {
        self._handleMessageAck(replyTo, message);
      } else {
        self._handleMostRecentMsgReply(replyTo, message);
      }
    } else {
      ...
      if (messageType === RTM_API_EVENTS.MESSAGE && !isUndefined(message.subtype)) {
        self.emit(makeMessageEventWithSubtype(message.subtype), message);
      }
    }
  });
};
...
RTMClient.prototype._handleMessageAck = function handleMessageAck(replyTo, message) {
  ...
  if (hasKey(this._msgResponseHandlers, replyTo)) {
    if (!message.ok) {
      ...
      promise.then(function handleMessageAckPromiseInner() {
        self._handleMsgResponse(replyTo, null, msg);
      });
    }
  } else {
