...
SlackDataStore.prototype.upsertChannelGroupOrDMById = function upsertChannelGroupOrDMById(id, obj) {
  ...
  if (firstChar === 'C') {
    ret = this.upsertChannel(obj);
  } else if (firstChar === 'G') {
    ret = this.upsertGroup(obj);
  } else if (firstChar === 'D') {
    ret = this.upsertDM(obj);
  }
...
SlackDataStore.prototype.getChannelOrGroupByName = function getChannelOrGroupByName(name) {
  return this.getChannelByName(name)
    .then(function getChannelByNamePromiseInner(channel) {
...
SlackDataStore.prototype.cacheRtmStart = function cacheRtmStart(data) {
  var self = this;
  return this.clear()
    .then(function cacheRtmStartClearPromiseInner() {
      var promises = [];
      ...
      return Promise.all(promises)
        .then(function cacheRtmStartAllPromiseInner() {
          return self.getUserById(data.self.id);
        })
        .then(function cacheRtmStartUserPromiseInner(user) {
          user.update(data.self);
          return self.setUser(user);
        })
        .then(self.setTeam(data.team));
    });
};
...
SlackDataStore.prototype.handleRtmMessage = function handleRtmMessage(
  ...
  if (isUndefined(handler)) return Promise.resolve();
  return new Promise(function handleRtmMessagePromiseInner(resolve, reject) {
    try {
      if (handler.length === 4) {
        resolve(handler(activeUserId, activeTeamId, self, message));
      } else {
        resolve(handler(self, message));
      }
    } catch (err) {
