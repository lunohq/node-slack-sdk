...
var setBaseChannelProperty = function setBaseChannelProperty(val, key) {
  return function setBaseChannelPropertyWrapped(dataStore, message) {
    return dataStore.getChannelGroupOrDMById(message.channel)
      .then(function setBaseChannelPropertyWrappedPromiseInner(obj) {
        var baseChannel = obj;
        var ret = Promise.resolve();
        if (baseChannel) {
          baseChannel[key] = val;
          ret = dataStore.upsertChannelGroupOrDMById(baseChannel.id, baseChannel);
        }
        return ret;
      });
  };
...
var handleChannelGroupOrDMMarked = function handleChannelGroupOrDMMarked(dataStore, message) {
  ...
  return dataStore.getChannelGroupOrDMById(message.channel)
    .then(function handleChannelGroupOrDMMarkedPromiseInner(obj) {
      var ret = Promise.resolve();
      var baseChannel = obj;
      if (baseChannel) {
        ...
        if (firstChar === 'C') {
          ret = dataStore.setChannel(baseChannel);
        } else if (firstChar === 'G') {
          ret = dataStore.setGroup(baseChannel);
        } else if (firstChar === 'D') {
          ret = dataStore.setDM(baseChannel);
        }
      }
      return ret;
    });
...
var handleRename = function handleRename(dataStore, message) {
  return dataStore.upsertChannelGroupOrDMById(message.channel.id, message.channel);
};
...
var handleLeave = function handleLeave(activeUserId, activeTeamId, dataStore, message) {
  ...
  return dataStore.getChannelGroupOrDMById(message.channel)
    .then(function handleLeavePromiseInner(baseChannel) {
      var ret = Promise.resolve();
      if (baseChannel) {
