...
var toggleReactionForMessage = function toggleReactionForMessage(dataStore, message, isAdded) {
  ...
  return dataStore.getChannelGroupOrDMById(item.channel)
    .then(function toggleReactionForMessagePromiseInner(channel) {
...
var toggleReaction = function toggleReaction(isAdded, dataStore, message) {
  ...
  if (itemType === 'file') {
    ret = toggleReactionForFile(dataStore, message, isAdded);
  } else if (itemType === 'file_comment') {
    ret = toggleReactionForFileComment(dataStore, message, isAdded);
  } else if (itemType === 'message') {
    ret = toggleReactionForMessage(dataStore, message, isAdded);
  }
  return ret;
};
